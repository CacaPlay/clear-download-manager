...
use crate::{
    ensure_public_network_resolution, html_attribute_values, html_page_title,
    job_uses_legacy_removed_provider, kill_process_tree, legacy_removed_provider_error,
    normalize_schedule_time, page_candidate_score, parse_public_http_url, read_currency_snapshot,
    read_download_activity, read_schedule, resolve_download_filename,
    resolve_extension_download_filename, resume_download_worker_when_idle,
    resume_media_worker_when_idle, resume_torrent_worker_when_idle, terminate_external_processes,
...
pub(crate) fn set_job_status(
    ...
    if matches!(status.as_str(), "queued" | "running")
        && job_uses_legacy_removed_provider(&connection, id)?
    {
...
pub(crate) fn create_download_schedule(
    ...
    if let Some(job_id) = job_id {
        ...
        if action == "resume" && job_uses_legacy_removed_provider(&connection, job_id)? {
            return Err(legacy_removed_provider_error());
        }
    }
    connection
