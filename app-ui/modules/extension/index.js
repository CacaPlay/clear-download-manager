...
function extensionPlaylistItem(item = {}, index = 0) {
  ...
  return {
    ...
    metadataUrl: String(item.metadataUrl || item.metadata_url || item.spotifyUrl || item.spotify_url || '').trim(),
    spotifyUrl: String(item.spotifyUrl || item.spotify_url || '').trim(),
    resolutionState: String(item.resolutionState || item.resolution_state || '').trim(),
    matchScore: Number(item.confidence || item.matchScore || item.match_similarity || 0),
    ...
    isrc: String(item.isrc || '').trim(),
    album: String(item.album || '').trim(),
    albumArtist: String(item.albumArtist || item.album_artist || '').trim(),
    releaseDate: String(item.releaseDate || item.release_date || '').trim(),
    trackNumber: item.trackNumber == null && item.track_number == null ? null : Number(item.trackNumber ?? item.track_number),
    discNumber: item.discNumber == null && item.disc_number == null ? null : Number(item.discNumber ?? item.disc_number),
    explicit: Boolean(item.explicit)
  };
}

function spotifyOutputMode(payload = {}) {
  const preferredFormat = String(payload.preferredFormat || '').toLowerCase();
  if (preferredFormat === 'mp3') return 'audio_mp3';
  if (preferredFormat === 'm4a') return 'audio_m4a';
  return 'audio_best';
}

function spotifyPlaylistFormatLabel(payload = {}) {
  const preferredFormat = String(payload.preferredFormat || '').toLowerCase();
  if (preferredFormat === 'mp3') return 'MP3 320 kbps';
  if (preferredFormat === 'm4a') return 'M4A';
  return 'Original / mejor audio disponible';
}

function resolvedSpotifyPlaylistItems(media = {}) {
  return (Array.isArray(media.items) ? media.items : [])
    .map((item, index) => extensionPlaylistItem({
      ...item,
      id: item.source_id || item.sourceId,
      selectedSourceUrl: item.selected_source_url || item.selectedSourceUrl || item.source_url || item.sourceUrl,
      metadataUrl: item.metadata_url || item.metadataUrl,
      spotifyUrl: item.spotify_url || item.spotifyUrl,
      resolutionState: item.resolution_state || item.resolutionState,
      ...
      confidence: item.match_similarity
    }, index))
    .filter((item) => item.sourceUrl);
}

async function queueResolvedSpotifySource(source, payload = {}) {
  const media = await invoke('resolve_spotify_source', { url: source });
  const items = resolvedSpotifyPlaylistItems(media);
  if (media?.kind === 'playlist' || items.length > 1) {
    return invoke('queue_playlist_selection', {
      playlistTitle: String(payload.playlistTitle || media.title || 'Playlist de Spotify'),
      items,
      format: spotifyPlaylistFormatLabel(payload)
    });
  }
  return invoke('queue_spotify_download', {
    url: source,
    title: media?.title || payload.title || 'Descarga de Spotify',
    thumbnail: media?.thumbnail || payload.thumbnail || '',
    format: spotifyOutputMode(payload),
...
async function queueExtensionSourceInBackground(source, payload = {}) {
  if (isSpotifyUrl(source)) return queueResolvedSpotifySource(source, payload);
...
async function processExtensionBridgeRequests() {
  ...
  try {
    ...
    if (manualPlaylist || urls.length > 1) {
      ...
      if (urls.some(isSpotifyUrl)) {
        await queueResolvedSpotifySource(urls[0], payload);
        await loadSnapshot();
        if (foreground) {
          showToast('Contenido de Spotify añadido a la cola.', 'success');
          render();
        }
        return;
      }
