...
function multimediaSettings(context = {}) {
  ...
  return `<div class="dm-settings-section-body dm-multimedia-settings">
    ...
    <article class="dm-settings-feature dm-media-session-settings ${session.useBraveCookies || cookiesPath ? 'is-ready' : 'is-pending'}"><div class="dm-settings-feature-head"><span>${dmIcon('shield', 20)}</span><div><strong>Sesión para YouTube y plataformas compatibles</strong><small>Se configura una sola vez y se aplica a análisis, descargas y playlists.</small></div><i>${session.useBraveCookies || cookiesPath ? 'Configurada' : 'Anónima'}</i></div><p class="dm-settings-message">${escapeHtml(sessionLabel)}</p><div class="dm-settings-feature-actions"><label class="dm-switch-row"><span><strong>Usar cookies de Brave</strong><small>Solo el perfil local, sin enviarlas a CacaTools ni a otro servidor.</small></span><input type="checkbox" data-dm-media-session-consent ${session.useBraveCookies ? 'checked' : ''}></label><button type="button" data-dm-choose-media-cookies>${dmIcon('file', 16)} Elegir cookies Netscape (.txt)</button>${cookiesPath ? `<button type="button" data-dm-clear-media-cookies>Quitar archivo</button>` : ''}</div><small class="dm-settings-message">Por seguridad se guarda la preferencia y la ruta local; el contenido del archivo no se copia a SQLite. Si reinstalas Windows o limpias AppData, usa nuevamente el selector.</small></article>
    <aside class="dm-settings-explanation"><strong>Preferencias persistentes activas</strong><span>CacaTools conserva salida, calidad, formato de playlist y la configuración de sesión. Las cookies no se leen sin consentimiento explícito.</span></aside>
  </div>`;
}

function integrationsSettings(context = {}) {
  const spotifyConnected = Boolean(context.spotifyAuth?.connected);
  return `<div class="dm-settings-section-body dm-infrastructure-settings">
    <article class="dm-settings-feature ${spotifyConnected ? 'is-ready' : 'is-pending'}">
      <div class="dm-settings-feature-head"><span>${dmIcon('audio', 20)}</span><div><strong>Spotify</strong><small>Metadatos oficiales para búsquedas y descargas de música.</small></div><i>${spotifyConnected ? 'Conectado' : 'Opcional'}</i></div>
      <p class="dm-settings-message">${spotifyConnected ? 'Spotify está conectado. El acceso se renueva automáticamente cuando sea necesario.' : 'Spotify no está conectado. YouTube y las descargas normales seguirán funcionando con normalidad.'}</p>
      <div class="dm-settings-feature-actions"><button type="button" data-dm-spotify-connect>${spotifyConnected ? 'Reconectar con Spotify' : 'Conectar con Spotify'}</button>${spotifyConnected ? '<button type="button" data-dm-spotify-disconnect>Desconectar</button>' : ''}</div>
      <small class="dm-settings-message">Se abrirá el navegador oficial de Spotify. CacaTools no almacena un Client Secret.</small>
    </article>
