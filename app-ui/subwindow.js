...
function platformInfo(source = state.source) {
  ...
  return ['http', host.replace(/^www\./, '') || 'Descarga HTTP'];
}

function platformLogo(source = state.source) {
  const platform = platformInfo(source);
  return `<div class="platform-slot" title="${escapeHtml(platform[1])}" aria-label="${escapeHtml(platform[1])}">${icon('link', 18)}</div>`;
}

function platformBadge(source = state.source) {
  const platform = platformInfo(source);
  return `<span class="platform-badge">${icon('link', 18)}<strong>${escapeHtml(platform[1])}</strong></span>`;
}
