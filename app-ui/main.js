...
const ICON_PATHS = Object.freeze({
  ...
  playlist: '<circle cx="3.8" cy="5.2" r="1.35" fill="currentColor" stroke="none"/><rect x="7" y="3.8" width="7.4" height="2.8" rx="1.4" fill="currentColor" stroke="none"/><circle cx="3.8" cy="12" r="1.35" fill="currentColor" stroke="none"/><rect x="7" y="10.6" width="6.3" height="2.8" rx="1.4" fill="currentColor" stroke="none"/><circle cx="3.8" cy="18.8" r="1.35" fill="currentColor" stroke="none"/><rect x="7" y="17.4" width="6.3" height="2.8" rx="1.4" fill="currentColor" stroke="none"/><circle cx="17" cy="12" r="4.35" fill="#4f9bff" stroke="none"/><path d="m15.65 9.8 3.55 2.2-3.55 2.2z" fill="#101a26" stroke="none"/>',
...
const demoPlaylist = [
  ...
  { id: 'p2', title: 'Mitski — My Love Mine All Mine', creator: 'LatinHype', duration: '2:19', selected: true, tone: 2, thumbnail: './app-ui/media-preview.svg' },
  { id: 'p3', title: 'Jace June — Come Home', creator: 'Jace June', duration: '2:49', selected: true, tone: 3, thumbnail: './app-ui/media-preview.svg' },
...
const appState = {
  ...
  spotifyAuth: previewMode ? { connected: false, state: 'disconnected' } : { connected: false, state: 'disconnected' },
  mediaAnalysis: null,
  spotifyAttributionUrl: '',
...
async function invoke(command, args = {}) {
  ...
  try {
    ...
    if (command === 'spotify_auth_status') {
      const state = result?.connected && appState.spotifyAuth?.state === 'connected' ? 'connected' : result?.connected ? 'authenticated' : 'disconnected';
      return { ...result, connected: state === 'connected', state };
    }
    if (command === 'spotify_login') {
      return { ...result, connected: false, state: result?.connected ? 'authenticated' : 'disconnected' };
    }
    if (command === 'spotify_logout') {
      appState.spotifyAuth = { connected: false, state: 'disconnected' };
      return result;
    }
    if (command === 'resolve_spotify_source') {
      appState.spotifyAuth = { ...(appState.spotifyAuth || {}), connected: true, state: 'connected' };
    }
    return result;
  } catch (error) {
    const raw = String(error?.message || error || '');
    if (command === 'resolve_spotify_source' && new RegExp(`${SPOTIFY_RESTRICTED_CODE}|active premium subscription required`, 'i').test(raw)) {
      appState.spotifyAuth = { ...(appState.spotifyAuth || {}), connected: false, state: 'authenticated_restricted' };
    } else if (command === 'resolve_spotify_source' && /spotify_not_connected|spotify.*(401|expired|invalid_grant)|HTTP 401/i.test(raw)) {
      appState.spotifyAuth = { ...(appState.spotifyAuth || {}), connected: false, state: SPOTIFY_AUTH_ERROR_STATE };
    }
    throw error;
  }
}
...
function isSpotifyUrl(value) {
  ...
  if (/^spotify:/i.test(source)) return true;
  try {
    const normalized = /^[a-z][a-z0-9+.-]*:\/\//i.test(source) ? source : `https://${source}`;
    const host = new URL(normalized).hostname.toLocaleLowerCase();
    return host === 'spotify.com' || host.endsWith('.spotify.com');
  } catch {
    return /(?:^|\.)spotify\.com(?:[\\/?#:]|$)/i.test(source);
  }
}
...
function friendlyError(error) {
  ...
  if (new RegExp(`${SPOTIFY_RESTRICTED_CODE}|active premium subscription required`, 'i').test(raw)) return SPOTIFY_RESTRICTED_MESSAGE;
  if (/spotify_not_connected|spotify.*(401|expired|invalid_grant)|HTTP 401/i.test(raw)) return 'La sesión de Spotify ya no es válida. Reconecta Spotify para continuar.';
  if (/spotify_disabled|Spotify está desactivado temporalmente/i.test(raw)) return SPOTIFY_DISABLED_MESSAGE;
  if (/provider_failed|spotify_engine_unavailable|spotify_engine_hash_mismatch|download_not_allowed/i.test(raw)) return 'No se encontró una versión disponible para descargar.';
...
async function checkForAppUpdate({ silent = false } = {}) {
  if (previewMode || appState.updaterCheckBusy || appState.updaterInstallBusy) return appState.availableUpdate;
...
function downloadsPageMarkup() {
  ...
  return renderDownloadManager({
    ...
    spotifyAuth: appState.spotifyAuth,
...
function bindEvents() {
  ...
  document.querySelector('.spotify-connect')?.addEventListener('click', async (event) => {
    if (previewMode) return;
    const button = event.currentTarget;
    button.disabled = true;
    try {
      appState.spotifyAuth = await invoke('spotify_login');
      showToast(appState.spotifyAuth?.state === 'authenticated' ? 'Spotify autenticado. La API se verificará al analizar una fuente.' : 'Spotify conectado correctamente', 'info');
    } catch (error) {
      showToast(`No se pudo conectar Spotify: ${String(error)}`, 'error');
    } finally {
      render();
    }
  });
  document.querySelector('.spotify-disconnect')?.addEventListener('click', async (event) => {
    if (previewMode) return;
    const button = event.currentTarget;
    button.disabled = true;
    try {
      ...
      showToast(`No se pudo desconectar Spotify: ${String(error)}`, 'error');
    } finally {
      render();
    }
  });
