...
const required = [
  ...
  'src-tauri/Cargo.toml', 'src-tauri/Cargo.lock', 'src-tauri/tauri.conf.json', 'src-tauri/src/lib.rs', 'src-tauri/src/main.rs', 'src-tauri/src/subwindows.rs', 'src-tauri/src/commands/subwindows.rs',
  'src-tauri/src/update_manager.rs', 'src-tauri/src/extension_bridge.rs',
  ...
  'scripts/validation/phase22_internal_workspace_smoke.py', 'scripts/validation/benchmark-spotify-pipeline.ps1', 'scripts/validation/browser-capture-smoke.mjs', 'scripts/validation/video-detector-smoke.mjs', 'scripts/version-check.mjs', 'scripts/build-extension.ps1', 'docs/extension/INSTALL.md', 'docs/extension/STORE-SUBMISSION.md', 'docs/extension/PRIVACY.md', 'docs/RELEASE-GUIDE.md',
...
const jsFiles = [
  ...appUiJsFiles, 'app-ui/player/player.js', 'scripts/build.mjs', 'scripts/generate-source-manifest.mjs',
...
for (const [source, token, label] of [
  ...
  [rust, 'spotdl_save_metadata', 'motor spotDL para metadatos'],
  [rust, 'verify_spotdl_binary', 'verificación SHA-256 de spotDL'],
  [rust, 'queue_spotify_download', 'cola aislada de Spotify'],
  [rust, 'provider_id', 'proveedor persistente de descarga'],
  [rust, 'sanitize_spotify_text', 'saneado centralizado de metadatos Spotify'],
  [rust, 'SPOTIFY_VERSION_TERMS', 'versiones musicales preservadas'],
  [rust, 'SPOTIFY_RESOLUTION_STATES', 'máquina de estados persistente'],
...
const hotfix7Gate = spawnSync(process.execPath, ['scripts/validation/phase24_1_hotfix7_playlist_spotify.mjs'], { encoding: 'utf8' });
if (hotfix7Gate.status !== 0) fail(hotfix7Gate.stderr || hotfix7Gate.stdout || 'Falló el gate Hotfix 7');
