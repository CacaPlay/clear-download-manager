...
$RequiredWebResources = @(
  ...
  "app-ui\assets\platforms\twitter.ico"
)
...
$WebHashes = [ordered]@{
  ...
  platformLogos = [ordered]@{
    youtube = (Get-FileHash (Join-Path $Root "dist\app-ui\assets\platforms\youtube.ico") -Algorithm SHA256).Hash.ToLowerInvariant()
    tiktok = (Get-FileHash (Join-Path $Root "dist\app-ui\assets\platforms\media_social_tiktok_icon_124256.ico") -Algorithm SHA256).Hash.ToLowerInvariant()
    pinterest = (Get-FileHash (Join-Path $Root "dist\app-ui\assets\platforms\pinterest_19134.ico") -Algorithm SHA256).Hash.ToLowerInvariant()
    instagram = (Get-FileHash (Join-Path $Root "dist\app-ui\assets\platforms\instagram_logo_icon_168715.ico") -Algorithm SHA256).Hash.ToLowerInvariant()
    facebook = (Get-FileHash (Join-Path $Root "dist\app-ui\assets\platforms\4202110facebooklogosocialsocialmedia-115707_115594.ico") -Algorithm SHA256).Hash.ToLowerInvariant()
    x = (Get-FileHash (Join-Path $Root "dist\app-ui\assets\platforms\twitter.ico") -Algorithm SHA256).Hash.ToLowerInvariant()
  }
}
